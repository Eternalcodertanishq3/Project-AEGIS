...
export function Sidebar({ activeItem, onNavigate }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        'flex flex-col h-[calc(100vh-3.5rem)] border-r border-slate-700/50 bg-slate-900/50 transition-all duration-300',
        collapsed ? 'w-16' : 'w-60'
      )}
    >
      {/* Collapse toggle */}
      <div className="flex items-center justify-end p-2 border-b border-slate-700/30">
        <button
              ...
              {items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'flex items-center w-full gap-3 px-2.5 py-2 rounded-md text-sm font-medium transition-all duration-150',
                      isActive
      ...
      {!collapsed && (
        <div className="p-3 border-t border-slate-700/30">
          <div className="flex items-center gap-2 text-slate-500">
            ...
            <span className="text-xs font-medium">AEGIS v0.1.0</span>
          </div>
        </div>
      )}
    </aside>
